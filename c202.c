...
void Stack_Init( Stack *stack ) {
	...
	stack->array = malloc(STACK_SIZE*sizeof(char));	//alokujeme pole o velikosti stacku
	stack->topIndex = -1;	//inicializujeme zasobnik jako prazdny
}
...
bool Stack_IsEmpty( const Stack *stack ) {
	return (stack->topIndex==-1 ? true : false);	//pokud je hodnota vrchniho prvku -1, znamena to ze je prazdny a vrati true, jinak false
}
...
bool Stack_IsFull( const Stack *stack ) {
	return (stack->topIndex == STACK_SIZE-1 ? true : false);	//pokud je hodnota topIndexu rovna maximalnimu poctu prvku (-1 kvuli inicializovani od 0), vrati true jinak false
}
...
void Stack_Top( const Stack *stack, char *dataPtr ) {
	if(Stack_IsEmpty(stack)){
		 ...
		 *dataPtr = stack->array[stack->topIndex];	//do promenne dataPtr predame hodnotu ktera je ulozena ja vrcholu zasobniku
	}
}
...
void Stack_Pop( Stack *stack ) {
	if(!Stack_IsEmpty(stack))
		stack->topIndex--;	//pokud neni stack prazdny, posuneme hodnotu topIndexu o 1 dolu
}
...
void Stack_Push( Stack *stack, char data ) {
	if(Stack_IsFull(stack)){
		...
		stack->topIndex++;	//zvysime hodnotu topIndexu
		stack->array[stack->topIndex] = data;	//ulozime na toto misto prislusna data
	}
}
...
void Stack_Dispose( Stack *stack ) {
	free(stack->array);		//uvolnime alokovane pole
	stack->array = NULL;	
	stack->topIndex = -1;	//inicializujeme zasobnik jako prazdny
}
